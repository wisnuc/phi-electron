  ...
  constructor (props) {
        ...
        const read = async (entries, downloadPath, dirUUID, driveUUID, task) => {
          for (let i = 0; i < entries.length; i++) {
            ...
            if (entry.type === 'directory') {
              ...
              const ep = validator.isUUID(driveUUID) ? `drives/${driveUUID}/dirs/${entry.uuid}`
                : `phy-drives/${driveUUID}?${querystring.stringify({ path: `${dirUUID}${entry.name}` })}`

              const children = validator.isUUID(driveUUID) ? (await serverGetAsync(ep)).entries : await serverGetAsync(ep)
              const newDirUUID = validator.isUUID(driveUUID) ? entry.uuid : `${dirUUID}${entry.name}/`

              this.push({ entries: children, downloadPath: entry.downloadPath, dirUUID: newDirUUID, driveUUID, task })
            } else {
    ...
    this.download = new Transform({
      ...
      transform: (x, callback) => {
        ...
        const ep = validator.isUUID(driveUUID) ? `drives/${driveUUID}/dirs/${dirUUID}/entries/${entry.uuid}`
          : `phy-drives/${driveUUID}`
        const qs = validator.isUUID(driveUUID) ? { name: entry.name } : { path: `${dirUUID}${entry.name}` }
