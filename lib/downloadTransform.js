  ...
  constructor (props) {
        ...
        const read = async (entries, downloadPath, dirUUID, driveUUID, domain, task) => {
          for (let i = 0; i < entries.length; i++) {
            ...
            if (entry.type === 'directory') {
              ...
              const ep = isPhy ? `phy-drives/${driveUUID}?${querystring.stringify({ path: `${dirUUID}${entry.name}` })}`
                : `drives/${driveUUID}/dirs/${entry.uuid}`

              const children = isPhy ? await serverGetAsync(ep) : (await serverGetAsync(ep)).entries
    ...
    this.download = new Transform({
      ...
      push (X) {
        ...
        entries.forEach((entry) => {
          if (entry.type === 'directory' || entry.finished) {
            ...
            this.pending.push({ entry, downloadPath, dirUUID, driveUUID, domain, task })
          }
        })
        this.schedule()
      },
      transform: (x, callback) => {
        ...
        const qs = isPhy ? { path: `${dirUUID}${entry.name}` } : { name: entry.name }
