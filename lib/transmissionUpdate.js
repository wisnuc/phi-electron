...
const sendMsg = () => {
  ...
  userTasks.sort((a, b) => b.completeSize / (b.size || 1) - a.completeSize / (a.size || 1)) // Descending by percentage
...
const taskSchedule = () => {
  if (Tasks.filter(t => !t.paused && t.state !== 'finished' && t.trsType === 'upload').length < 5) {
    const task = Tasks.find(t => t.waiting && t.paused && t.state !== 'finished' && t.trsType === 'upload')
    if (task) {
      task.wait(false)
      task.run()
      taskSchedule()
