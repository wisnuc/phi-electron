...
const onTrayData = (event) => {
  ...
  if (store.getState().login.state === 'LOGIN') {
    console.log('store.getState()', store.getState().login.device.boot.data.boundVolume)
    const { device, user } = store.getState().login
    logged = true
    try {
      ...
      storageText = i18n.__('Storage Usage %s %s', prettysize(deviceSize - free), prettysize(deviceSize))
      storageBar = 1 - free / deviceSize
      deviceName = device.mdev.stationName
    } catch (e) {
      console.error('parse Tray Data error')
      deviceName = 'N2'
    }
    deviceStatus = i18n.__('Online and LAN Mode')
...
const onAboutData = (event) => {
  const data = {
    text: [
      ['name', 'N2'],
      ['model', '斐讯NAS'],
      ['version', `${i18n.__('Version:')} ${app.getVersion()}`],
      ['copyrightCn', `版权所有©上海斐讯数据通信有限公司${new Date().getFullYear()}`],
