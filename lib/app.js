...
const updateTray = () => {
  ...
  const icon = path.join(global.rootPath, 'public/assets/images/icon.ico')
  ...
  tray.on('click', () => {
    getMainWindow().show()
    if (process.platform === 'linux') showTrayWindow()
  })
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    // { label: i18n.__('Show'), type: 'normal', click: () => showWindow(), icon },
