  ...
  constructor (props) {
          ...
          for (let i = 0; i < entries.length; i++) {
            ...
            if (stat.isDirectory()) {
              ...
              const uuid = domain === 'phy'
                ...
                : res && res.uuid
              // console.log('create fold, domain, res=======\n', domain, uuid, res, '\n\n')
    ...
    this.hash = new Transform({
      ...
      transform: (x, callback) => {
        ...
        readXattr(entry, (error, attr) => {
          ...
          if (!(policy.mode === 'merge' || policy.mode === 'overwrite') && task.isNew && !retry) {
    ...
    this.diff = new Transform({
      ...
      transform: (X, callback) => {
        const diffAsync = async (local, driveUUID, dirUUID, domain, task) => {
          ...
          const ep = isPhy ? `phy-drives/${driveUUID}?${querystring.stringify({ path: dirUUID })}`
    ...
    this.upload = new Transform({
      ...
      push (X) {
        ...
        X.forEach((x) => {
          if (x.type === 'directory') {
            ...
            const i = this.pending.findIndex(p => p.length < 256 &&
      ...
      transform: (X, callback) => {
            ...
            if (!p.sha) {
              ...
              const ts = new stream.Transform({
                ...
                flush (cb) {
                  ...
                  if (i === parts.length - 1) setXattr(entry, { parts }, err => err && console.error('set xattr error', err))
        ...
        // console.log('uploading transform domain', domain)
