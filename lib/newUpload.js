...
const readUploadInfoAsync = async (entries, dirUUID, driveUUID) => {
  ...
  const isDrive = validator.isUUID(driveUUID)
  const ep = isDrive ? `drives/${driveUUID}/dirs/${dirUUID}`
    : `phy-drives/${driveUUID}?${querystring.stringify({ path: dirUUID })}`
  const listNav = await serverGetAsync(ep)
  // console.log('readUploadInfoAsync listNav ep=======\n', ep, listNav)
  const remoteEntries = isDrive ? listNav.entries : listNav
  ...
  if (filtered.length) {
    filtered.forEach((f, i) => {
      const taskUUID = UUID.v4()
      const newEntries = [f.entry]
      const mode = f.policy ? f.policy : 'normal'
      const checkedName = mode === 'rename' ? f.checkedName : undefined
      const policies = [{ mode, checkedName, remoteUUID: f.remoteUUID }]
      createTask(taskUUID, newEntries, dirUUID, driveUUID, f.entryType, createTime, newWork, policies)
    })
  }
  */
