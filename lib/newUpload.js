...
const uploadFirmHandle = (event, args) => {
  console.log('on uploadFirmHandle', args)
  const { absPath, session } = args
  fs.lstat(path.resolve(absPath), (err, stat) => {
    console.log('stat', stat)
    if (err) event.sender.send('UPLOAD_FIRM_RESULT', { success: !err, reason: err, session })
    else {
      uploadFirm(event, absPath, stat.size, session, (error) => {
        console.log('uploadFirm result', error)
        event.sender.send('UPLOAD_FIRM_RESULT', { success: !error, reason: error, session })
      })
    }
  })
}
