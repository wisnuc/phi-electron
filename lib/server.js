...
const phyRemoteUploadOrMkdir = (ep, params) => {
  const url2 = `${cloudAddress}/ResourceManager/app/pipe/resource`
  const data = {
    verb: 'POST',
    urlPath: `/${ep}`,
  ...
  console.log('phyRemoteUpload', data)
  const qs = querystring.stringify({ deviceSN, data: JSON.stringify(data) })
  return request.post(`${url2}?${qs}`).set('Authorization', token)
}
...
class UploadMultipleFiles {
  ...
  phyUpload () {
    this.handle = cloud ? phyRemoteUploadOrMkdir(`phy-drives/${this.driveUUID}`, { path: this.dirUUID })
      : apost(`phy-drives/${this.driveUUID}?${querystring.stringify({ path: this.dirUUID })}`, null, true)
...
const createFold = (driveUUID, dirUUID, dirname, localEntries, policy, domain, callback) => {
  let handle
  if (domain === 'phy') {
    if (cloud) handle = phyRemoteUploadOrMkdir(`phy-drives/${driveUUID}`, { path: dirUUID }).field('directory', dirname)
    else {
