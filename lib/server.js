...
const cloudAddress = process.env.CLOUD_TEST === 'dev' ? 'http://sohon2dev.phicomm.com' : 'http://sohon2test.phicomm.com'
  ...
  const handle = adownload(ep, qs)
    ...
    .on('response', (res) => {
      if (res.status !== 200 && res.status !== 206) {
        ...
        console.log('download res not octet-stream======\n', res)
...
class UploadMultipleFiles {
  ...
  normalUpload () {
    ...
    this.handle.on('error', (err) => {
      console.log('normalUpload error===\n', err)
      this.finish(err)
    })
...
const downloadFile = (entry, downloadPath, callback) => {
  ...
  fs.access(filePath, (error) => {
    if (error) {
      ...
      const qs = dirUUID === 'media' ? { alt: 'data' } : { name: fileName }
...
const uploadFirm = (event, absPath, size, session, callback) => {
  console.log('uploadFirm', absPath, size, session)
  ...
  const countRead = () => {
    ...
    event.sender.send('FIRM_PROCESS', { process: completeSize / size, session })
  }

  let countReadHandle
  rs.on('open', () => {
    countReadHandle = setInterval(countRead, 2000)
  })
