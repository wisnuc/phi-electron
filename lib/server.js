...
const downloadReq = (ep, fileName, filePath, callback) => {
  ...
  const handle = adownload(ep, { name: fileName })
    ...
    .on('response', (res) => {
      // console.log('downloadReq res', res.text)
      if (res.status !== 200 && res.status !== 206) {
        ...
        handle.abort()
        callback(e)
      } else if (cloud && res.type !== 'application/octet-stream') {
...
class UploadMultipleFiles {
  ...
  upload () {
    const isDrive = validator.isUUID(this.driveUUID)
    if (isDrive) this.normalUpload()
    else this.phyUpload()
  }
...
class DownloadFile {
  ...
  download () {
    ...
    this.handle
      ...
      .on('response', (res) => {
        console.log('downloadReq res', res)
